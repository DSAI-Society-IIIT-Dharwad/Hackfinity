...
class SmartMover(Node):
    ...
    def listener_callback(self, msg):
        ...
        # Downsample 360 → 30 rays
        self.lidar_state = processed[::12]
    def control_loop(self):
        cmd = Twist()

    # 🔥 SAFETY LAYER (DO NOT REMOVE)
        if self.mindist < 0.35:
            self.target_speed = 0.0
            self.target_turn = 0.75

        else:
        # 🔥 DRL WILL CONTROL HERE (for now dummy logic)
        
        # TEMP: simulate DRL output (replace later)
    ...
    # Smooth acceleration
        ...
        self.get_logger().info(
            f"Speed: {self.current_speed:.2f}, Turn: {self.current_turn:.2f}"
        )
