...
# Front 180° (indices 270 → 360 and 0 → 90)
        front_180 = processed[270:] + processed[:90]
        self.front_scan = front_180
    def control_loop(self):
        cmd = Twist()

        # Decision logic
        # 🔥 SAFETY FIRST (same as before)
        if self.mindist < 0.35:
            self.target_speed = 0.0
            self.target_turn = 0.75

        else:
    # 🔥 Find furthest direction in front 180°
            front_array = np.array(self.front_scan)
            max_index = int(np.argmax(front_array))

    # Convert index → angle (-90° to +90°)
            angle = (max_index - 90) / 90.0   # normalize to [-1, 1]

    # 🔥 Smooth turning (small, stable)
            self.target_turn = 0.5 * angle
